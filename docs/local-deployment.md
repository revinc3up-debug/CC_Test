...
2. 添加订阅：晚点LatePost、36氪、界面新闻、财新网、Tech星球、雷峰网、量子位（更全的清单见
   `docs/source-catalog.md`，能走原生 RSS 的优先原生、别都堆到微信侧）；
3. 添加频率放慢（每次间隔几分钟），避免触发微信侧封控；
4. 订阅数据在 `./data/we-mp-rss/`，**迁移云端时整个目录带走即可保留授权与订阅**。

**RSS 阅读器 / 原生订阅**：`examples/sourcing/feeds.opml` 可直接导入，一键订阅 36氪/虎嗅/
钛媒体/IT之家/少数派/晚点等原生或 RSSHub 源——这些零账号封控、不占门控配额，是日常拉取主力。
